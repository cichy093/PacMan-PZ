...
public class GameCompleteComp extends JComponent {
	private static final long serialVersionUID = -8678115443009583291L;
	
	private GameCompleteModel model;
	
	public GameCompleteComp() {
	...
	private void drawBackground(Graphics graphics) {
		Image background = new ImageIcon(this.getClass().getResource("/img/game_complete.jpg")).getImage();
	...
	protected String[] prepareTexts(int time, int points) {
		String[] txts = PacmanConstants.gameCompleteTexts.clone();
		txts[0] += new SimpleDateFormat("mm:ss.S").format((long)(time));
		txts[1] += String.valueOf(points);
	...
	protected void paintComponent(Graphics g) {
		...
		int width = getWidth();
		int height = (getHeight() - PacmanConstants.spaceBetweenTexts * PacmanConstants.gameCompleteCnt) / 2;
		...
		String[] textsToDraw = prepareTexts(model.getTotalTime(), model.getTotalPoints());
		
		int i = 1;
		for (String x : textsToDraw) {
			if (!x.equals("")) {
				if (i != PacmanConstants.gameCompleteCnt)
	...
	public void setModel(GameCompleteModel model) {
		if (this.model == null) {
			this.model = model;
		}
	}
