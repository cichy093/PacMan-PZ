...
public class GameOverComp extends JComponent {
	...
	private void drawBackground(Graphics graphics) {
		Image background = new ImageIcon(this.getClass().getResource("/img/game_over.jpg")).getImage();
	...
	protected String[] prepareTexts(int time, int points) {
		String[] txts = PacmanConstants.gameOverTexts;
		
		txts[2] = "Total Points: "+ String.valueOf(points);
		txts[3] = "Total Time: "+ new SimpleDateFormat("mm:ss.S").format((long)(time));
	...
	protected void paintComponent(Graphics g) {
		...
		int width = getWidth();
		int height = ((int)(getHeight()*0.8) - PacmanConstants.spaceBetweenTexts * PacmanConstants.gameOverCnt) / 2;
		...
		for (String x : textsToDraw) {
			if (!x.equals("")) {
				if (i != PacmanConstants.gameOverCnt)
