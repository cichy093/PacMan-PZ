...
public class IngameMenuComp extends JComponent {
	...
	private void drawBackground(Graphics graphics) {
		Image background = new ImageIcon(this.getClass().getResource("/img/pause_menu.jpg")).getImage();
	...
	protected void paintComponent(Graphics g) {
		...
		int width = getWidth();
		int height = (getHeight() - PacmanConstants.spaceBetweenTexts * PacmanConstants.ingameMenuCnt) / 2;
		...
		int i = 0;
		for (String x : PacmanConstants.ingameMenuTexts) {
	...
	public void setModel(IngameMenuModel model) {
		if (this.model == null) {
			this.model = model;
		}
	}
